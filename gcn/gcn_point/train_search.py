...
parser.add_argument('--batch_size', type=int, default=28, help='batch size') # 28 16 24
parser.add_argument('--batch_increase', default=4, type=int,
                    help='how much does the batch size increase after making a decision') # 4
...
parser.add_argument('--warmup_dec_epoch', type=int, default=16, help='warmup decision epoch') # 9 16
parser.add_argument('--decision_freq', type=int, default=21, help='decision freq epoch') # 7 21
...
def edge_decision(type, alphas, selected_idxs, candidate_flags,
                  curstage_selected_idxs, curstage_candidate_flags,
                  probs_history, epoch, model, args,
                  switches, decide_edge):
...
def main():
    ...
    num_to_drop = [2, 2, 3] # primitives to drop after each epoch
    ...
    for sp in range(len(num_to_drop)): # 3 search stage
        ...
        model = Network(args.init_channels, n_classes, args.num_cells + add_cells[sp], criterion,
                        args.n_steps, in_channels=args.in_channels,
                        emb_dims=args.emb_dims, dropout=args.dropout, k=args.k,
                        switches=switches, switch_on=switch_on,
                        normal_selected_idxs=normal_selected_idxs, 
                        normal_candidate_flags=normal_candidate_flags, 
                        curstage_selected_idxs=curstage_selected_idxs, 
                        curstage_candidate_flags=curstage_candidate_flags).cuda()
