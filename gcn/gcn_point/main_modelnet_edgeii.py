...
from model_edgeii import NetworkModelNet40 as Network
...
parser.add_argument('--batch_size', type=int, default=32, help='batch size') # 32 8
parser.add_argument('--use_sgd', type=int, default=1, help='Use SGD') # 0
parser.add_argument('--learning_rate', type=float, default=0.001, metavar='LR',
                    help='learning rate (default: 0.001, 0.1 if using sgd)') # 0.0005
...
parser.add_argument('--fixed_drop_path_prob', action='store_true', default=False, help='disable drop path probability schedule')
parser.add_argument('--test_inference_time', action='store_true', default=False, help='test inference time with batch size 1')
...
def train():
    ...
    for epoch in range(args.epochs):
        ...
        if not args.disable_scheduler:
            ...
            if args.use_sgd:
                logging.info('epoch %d lr %e', epoch, args.learning_rate*100)
            else:
                logging.info('epoch %d lr %e', epoch, args.learning_rate)

        if not args.fixed_drop_path_prob:
...
def infer(test_queue, model, criterion):
    ...
    with torch.no_grad():
        for i, (data, label) in enumerate(test_queue):
            ...
            if args.test_inference_time: t_inference = time.time()
            out, out_aux = model(data)
            if args.test_inference_time: logging.info("Inference time: {:.4f}s".format(time.time() - t_inference))
...
if __name__ == '__main__':
    ...
    if args.test_inference_time:
        test_queue = DataLoader(ModelNet40(partition='test', num_points=args.num_points, data_dir=args.data),
                            num_workers=1, batch_size=2, shuffle=False, drop_last=False)
    else:
        test_queue = DataLoader(ModelNet40(partition='test', num_points=args.num_points, data_dir=args.data),
