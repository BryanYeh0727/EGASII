...
class Cell(nn.Module):
    def __init__(self, genotype, C_prev_prev, C_prev, C, k=9, d=1):
        ...
        self.preprocess1 = BasicConv([C_prev, C*4], 'relu', 'batch', bias=False)
    ...
    def _compile(self, C, op_names, indices, concat):
        ...
        op = OPS["ginii"](C*4, 1, self.d)
        self._ops += [op]
        # for name, index in zip(op_names, indices):
        #     op = OPS[name](C, 1, self.d)
        #     self._ops += [op]
        # self._indices = indices

    def forward(self, s0, s1, drop_prob, x_0):
        edge_index = self.dilated_knn_graph(s0)
        # edge_index = self.dilated_knn_graph(s1)
        ...
        #     op1 = self._ops[2 * i]
        #     op2 = self._ops[2 * i + 1]
        #     h1 = op1(h1, x_0, edge_index)
        #     h2 = op2(h2, x_0, edge_index)
...
class NetworkModelNet40(nn.Module):
    ...
    def __init__(self, C, num_classes, num_cells, auxiliary, genotype, stem_multiplier=3,
        ...
        self.preprocess = BasicConv([in_channels, C*4], 'relu', 'batch', bias=False)
    ...
    def forward(self, x):
        ...
        for i, cell in enumerate(self.cells):
            s0, s1 = s1, cell(s0, s1, self.drop_path_prob, x_0)
